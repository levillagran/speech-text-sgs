...
var Artyom = (function () {
  ...
  Artyom.prototype.detectErrors = function () {
    ...
    if (window.location.protocol != "https:") {
      console.warn(
        "Warning: artyom is being executed using the '" +
          window.location.protocol +
          "' protocol. The continuous mode requires a secure protocol (HTTPS)"
      );
    }
    return false;
  };
  ...
  Artyom.prototype.execute = function (voz) {
    ...
    if (_this.ArtyomProperties.name) {
      if (voz.indexOf(_this.ArtyomProperties.name) != 0) {
        _this.debug(
          'Artyom requires with a name "' +
            _this.ArtyomProperties.name +
            "\" but the name wasn't spoken.",
          "warn"
        );
        return;
      }
      ...
      if (event.error == "not-allowed") {
        ...
        if (event.timeStamp - start_timestamp < 100) {
          _this.triggerEvent(_this.ArtyomGlobalEvents.ERROR, {
            code: "info-blocked",
            message:
              "Artyom needs the permision of the microphone, is blocked.",
          });
        } else {
          _this.triggerEvent(_this.ArtyomGlobalEvents.ERROR, {
            code: "info-denied",
  ...
  Artyom.prototype.getVoice = function (languageCode) {
    ...
    if (!voiceIdentifiersArray) {
      console.warn(
        "The providen language " +
          languageCode +
          " isn't available, using English Great britain as default"
      );
      ...
      if (message.length > artyom_say_max_chunk_length) {
        ...
        naturalReading.forEach(function (chunk, index) {
          ...
          if (chunk.length > artyom_say_max_chunk_length) {
            ...
            var temp_processed = _this.splitStringByChunks(
              chunk,
